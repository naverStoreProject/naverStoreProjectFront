...
import type { routeType , OffcanvasData } from '@/components/layouts/header/header'
...
export const useOffcanvasStore = defineStore('offcanvas', {
  state: () => ({
    //오프캔버스 관리
    stack: [] as OffcanvasData[],
    //각 오프캔버스의 visible 관리
    isVisibleStates: new Map<number, boolean>(),
  }),
  actions: {
    //오프캔버스 열기 (클릭)
    open(item: routeType, data?: any) {
      this.stack.push({ routeType: item, data })
      setTimeout(() => {
