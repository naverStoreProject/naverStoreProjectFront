  ...
  <div class="p-4 space-y-6 text-sm">
    ...
    <div>
      <div class="flex justify-between items-center mb-2">
        <div class="text-gray-500 text-xs">최근 검색어</div>
      ...
      <div class="text-sm space-y-1">
        ...
        >
          <div>
            ...
            <span>{{ word }}</span>
            <span class="text-gray-400 text-xs">{{ getToday() }}</span>
          </div>
          <button class="text-gray-400 text-xs" @click="removeRecent(i)">×</button>
        </div>
      </div>
    </div>
    ...
    <!-- 추천 검색어 -->
    <div>
      <div class="text-gray-500 text-xs mb-2">추천 검색어</div>
      <div class="flex flex-wrap gap-2">
        <BaseBtn v-for="(tag, i) in recommendedTags" :key="i" :label="tag" rounded="full" size="sm" class="bg-surface-50" />
      </div>
    </div>

    <!-- 오늘의 혜택 -->
    <div class="space-y-2">
      <div class="text-gray-500 text-xs">오늘의 혜택</div>
      <div class="flex gap-2 overflow-x-auto">
        <BaseBtn v-for="(boon, i) in todayBoons" :key="i" :label="boon"rounded="full" size="sm" color="base" />
      </div>
    </div>
    ...
    <div>
      ...
      <div>
        <BaseBtn label="전체" rounded="full" size="sm" class="bg-primary-400" />
        <div class="grid grid-cols-2 gap-y-2 mt-2">
          ...
          >
            <span class="text-red-600 font-semibold w-4">{{ index + 1 }}</span>
            <span>{{ item }}</span>
          </div>
        </div>
      </div>
...
const recommendedTags = [
  '나이키 운동화', '휠라 운동화', '아식스 운동화', '뉴발란스 운동화',
  '나이키신발', '나이키', '창문형에어컨', '호카'
]
const todayBoons = [ '브랜드데이 미소페 | 할인 ~53%', '신상위크 HUNTER | 할인 ~43%' ]
const popularSearches = [
  '호카', '뉴발란스운동화', '아식스젤카야노', '러닝화', '나이키러닝화',
  '살로몬메리제인', '우포스쪼리', '노바블라스트5', '러닝화', '나이키슬리퍼'
]
